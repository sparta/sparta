...
FixEmitSurfMflow::FixEmitSurfMflow(SPARTA *sparta, int narg, char **arg) :
...
FixEmitSurfMflow::~FixEmitSurfMflow()
{
...
void FixEmitSurfMflow::grid_changed()
{
  create_tasks();
...
void FixEmitSurfMflow::create_task(int icell)
{
  ...
  for (i = 0; i < nsurf; i++) {
    ...
    if (!subsonic) {
      ...
      std::cout << "Ntarget value is " << "tasks[ntask].ntarget" << std::endl;
...
void FixEmitSurfMflow::perform_task()
{
...
void FixEmitSurfMflow::subsonic_inflow()
{
...
void FixEmitSurfMflow::subsonic_sort()
{
  int i,icell;
...
void FixEmitSurfMflow::subsonic_grid()
{
  ...
  double vstreamold[3];
  double normal_vel_dot, normal_vold_dot;
  Surf::Line *lines = surf->lines;
  Surf::Tri *tris = surf->tris;
  ...
  for (int i = 0; i < ntask; i++) {
    ...
    vstreamold[0]   = vstream[0];
    vstreamold[1]   = vstream[1];
    vstreamold[2]   = vstream[2];
    ...
    normal_vold_dot  = vstreamold[0]*normal[0] + vstreamold[1]*normal[1] + vstreamold[2]*normal[2];

    if (np == 0 || normal_vold_dot == 0.0) {
      vstream[0] = vmin * normal[0];
      vstream[1] = vmin * normal[1];
      vstream[2] = vmin * normal[2];
    } else {
      vstream[0] = mv[0] / masstot;
      vstream[1] = mv[1] / masstot;
      vstream[2] = mv[2] / masstot;

      vstream[0] = average_coef*vstream[0] + (1-average_coef)*vstreamold[0];
      vstream[1] = average_coef*vstream[1] + (1-average_coef)*vstreamold[1];
      vstream[2] = average_coef*vstream[2] + (1-average_coef)*vstreamold[2];
      // use the old dot product if the new dot product is less than vmin
      if (normal_vel_dot < vmin){
        vstream[0] = vstreamold[0];
        vstream[1] = vstreamold[1]; 
        vstream[2] = vstreamold[2];
      } 
    }
    // normal points to computational domain
    normal_vel_dot  = vstream[0]*normal[0] + vstream[1]*normal[1] + vstream[2]*normal[2]; //Eq. 13 in Lei2017 

    if (subsonic_style == MFLOW) {
      nsubsonic = mflow/(species[0].mass * (normal_vel_dot) * inlet_area); //Eq. 4 and Eq. 2 in Lei2017
      //std::cout << "Mflow value is:" << mflow << ", and nsubsonic is " << nsubsonic << std::endl;
      tasks[i].nrho     = nsubsonic;
      temp_thermal_cell = tsubsonic;
    } else {
      error->all(FLERR,"Illegal fix emit/surf_mflow_implicit command");
    }
...
void FixEmitSurfMflow::grow_task()
{
  int oldmax = ntaskmax;
  ntaskmax += DELTATASK;
...
int FixEmitSurfMflow::option(int narg, char **arg)
{
  if (strcmp(arg[0],"n") == 0) {
  ...
  if (strcmp(arg[0],"subsonic") == 0) {
    if (4 > narg) error->all(FLERR,"Illegal fix emit/face massflow command");
    subsonic = 1;
    subsonic_style = MFLOW;
    // mass flow 
    mflow = input->numeric(FLERR,arg[1]);
    if (mflow < 0.0) error->all(FLERR,"Illegal fix emit/face massflow command");
    // temperature
    if (strcmp(arg[2],"NULL") == 0) error->all(FLERR,"Illegal fix emit/face command: inlet temperature must be defined");
    else {
    ...
    // inlet area
    inlet_area  = input->numeric(FLERR,arg[3]);
    if (inlet_area <= 0.0) error->all(FLERR,"Illegal fix emit/face command: area must be positive");
    // minimum velocity
    vmin = input->numeric(FLERR,arg[4]);
    if (vmin < 0.0) error->all(FLERR,"Illegal fix emit/face command: vmin must be non-negative");
    // average window
    if (strcmp(arg[5],"NULL") == 0) average_window = 0.0;
    else {
      average_window = input->numeric(FLERR,arg[5]);
      if (average_window < 0.0)
        error->all(FLERR,"Average window cannot be negative");
      average_coef = 1.0 / (average_window+1.0);
    }
    return 6;
  }
  error->all(FLERR,"Illegal fix emit/surf_mflow_implicit command");
