       ...
       int scaled, int permute, int *nbuf, int usecollective)
{
  ...
  *nbuf = copy_size + scratch_size;
  ...
  num = 0;
  factor_3d(nfast,&num,list);
  for (i = 0; i < num; i++)
    if (list[i] != 2 && list[i] != 3 && list[i] != 5) flag = 1;
  num = 0;
  factor_3d(nmid,&num,list);
  for (i = 0; i < num; i++)
    if (list[i] != 2 && list[i] != 3 && list[i] != 5) flag = 1;
  num = 0;
  factor_3d(nslow,&num,list);
  for (i = 0; i < num; i++)
    if (list[i] != 2 && list[i] != 3 && list[i] != 5) flag = 1;
...
void factor_3d(int n, int *num, int *list)
{
  if (n == 1) {
    return;
  }
    ...
    *list = 2;
    (*num)++;
    factor_3d(n/2,num,list+1);
  }
  else if (n % 3 == 0) {
    *list = 3;
    (*num)++;
    factor_3d(n/3,num,list+1);
  }
  else if (n % 5 == 0) {
    *list = 5;
    (*num)++;
    factor_3d(n/5,num,list+1);
  }
  else if (n % 7 == 0) {
    *list = 7;
    (*num)++;
    factor_3d(n/7,num,list+1);
  }
  else if (n % 11 == 0) {
    *list = 11;
    (*num)++;
    factor_3d(n/11,num,list+1);
  }
  else if (n % 13 == 0) {
    *list = 13;
    (*num)++;
    factor_3d(n/13,num,list+1);
  }
  else {
    *list = n;
    (*num)++;
                    ...
                    struct fft_plan_3d *plan)
{
  ...
  if ((total1 > nsize) || (total2 > nsize) || (total3 > nsize)) return;
#endif
