...
ComputeStyle(react/isurf/grid/kk,ComputeReactISurfGridKokkos)

#else

#ifndef SPARTA_COMPUTE_REACT_ISURF_GRID_KOKKOS_H
#define SPARTA_COMPUTE_REACT_ISURF_GRID_KOKKOS_H
...
class ComputeReactISurfGridKokkos : public ComputeReactISurfGrid {
 public:
  ComputeReactISurfGridKokkos(class SPARTA *, int, char **);
  ComputeReactISurfGridKokkos(class SPARTA *);
...
/* ----------------------------------------------------------------------
   ...
   mirrors ComputeReactISurfGrid::surf_tally(); per-surf tally, compressed to
   the host in tallyinfo(); post-processing (collate to per-grid) is on host
------------------------------------------------------------------------- */
 ...
 private:
  DAT::t_int_2d d_reaction2col;     // [nreaction][ntotal], only if rpflag
