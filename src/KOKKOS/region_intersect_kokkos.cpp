...
int RegIntersectKokkos::flatten_region_kokkos(tdual_region_token_1d &k_tokens_out)
{
  ...
  for (int i = 0; i < nregion; i++) {
    ...
    if (i) k_tokens.view_host()[ntoken++].type = RKK_TOK_INTERSECT;
  }
  ...
  if (depth < 0)
    error->all(FLERR,"Internal error flattening region intersect for the KOKKOS package");
  if (depth > RKK_MAX_STACK) {
    char str[128];
    snprintf(str,sizeof(str),"Region intersect is nested too deeply for the KOKKOS package "
