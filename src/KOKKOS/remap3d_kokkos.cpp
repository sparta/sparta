...
RemapKokkos3d<DeviceType>::RemapKokkos3d(SPARTA *sparta) : Pointers(sparta)
{
  plan = nullptr;
}

template<class DeviceType>
RemapKokkos3d<DeviceType>::RemapKokkos3d(SPARTA *sparta, MPI_Comm comm,
...
template<class DeviceType>
RemapKokkos3d<DeviceType>::~RemapKokkos3d()
{
...
template<class DeviceType>
void RemapKokkos3d<DeviceType>::perform(typename FFT_AT::t_FFT_SCALAR_1d d_in, typename FFT_AT::t_FFT_SCALAR_1d d_out, typename FFT_AT::t_FFT_SCALAR_1d d_buf)
{
...
template<class DeviceType>
void RemapKokkos3d<DeviceType>::remap_3d_kokkos(typename FFT_AT::t_FFT_SCALAR_1d d_in, typename FFT_AT::t_FFT_SCALAR_1d d_out, typename FFT_AT::t_FFT_SCALAR_1d d_buf,
...
template<class DeviceType>
struct remap_plan_3d_kokkos<DeviceType>* RemapKokkos3d<DeviceType>::remap_3d_create_plan_kokkos(
  MPI_Comm comm,
  int in_ilo, int in_ihi, int in_jlo, int in_jhi,
  int in_klo, int in_khi,
  int out_ilo, int out_ihi, int out_jlo, int out_jhi,
  ...
  int usecollective, int usegpu_aware)
{
  ...
  if (!plan->usecollective) {
    ...
    if (nrecv) {
      if (permute == 0)
        plan->unpack = PackKokkos3d<DeviceType>::unpack_3d;
      else if (permute == 1) {
        if (nqty == 1)
          plan->unpack = PackKokkos3d<DeviceType>::unpack_3d_permute1_1;
        else if (nqty == 2)
          plan->unpack = PackKokkos3d<DeviceType>::unpack_3d_permute1_2;
        else
          plan->unpack = PackKokkos3d<DeviceType>::unpack_3d_permute1_n;
      }
      else if (permute == 2) {
        if (nqty == 1)
          plan->unpack = PackKokkos3d<DeviceType>::unpack_3d_permute2_1;
        else if (nqty == 2)
          plan->unpack = PackKokkos3d<DeviceType>::unpack_3d_permute2_2;
        else
          plan->unpack = PackKokkos3d<DeviceType>::unpack_3d_permute2_n;
      }
...
template<class DeviceType>
void RemapKokkos3d<DeviceType>::remap_3d_destroy_plan_kokkos(struct remap_plan_3d_kokkos<DeviceType> *plan)
{
...
namespace SPARTA_NS {
template class RemapKokkos3d<SPADeviceType>;
#ifdef SPARTA_KOKKOS_GPU
template class RemapKokkos3d<SPAHostType>;
#endif
}
