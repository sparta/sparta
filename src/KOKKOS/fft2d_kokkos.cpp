  ...
  Pointers(sparta)
{
  ...
  if (ngpus > 0 && execution_space == Device)
    sparta->error->all(FLERR,"Cannot use the NVPL FFT library with Kokkos on GPUs");
       ...
       int nthreads, int usegpu_aware)
{
  ...
  *nbuf = copy_size + scratch_size;
  ...
  plan->desc_slow->set_value(oneapi::mkl::dft::config_param::NUMBER_OF_TRANSFORMS, plan->total2/nslow);
  plan->desc_slow->set_value(oneapi::mkl::dft::config_param::FWD_DISTANCE, plan->length2);
  plan->desc_slow->set_value(oneapi::mkl::dft::config_param::BWD_DISTANCE, plan->length2);
