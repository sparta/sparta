...
ComputeStyle(gas/reaction/grid/kk,ComputeGasReactionGridKokkos)

#else

#ifndef SPARTA_COMPUTE_GAS_REACTION_GRID_KOKKOS_H
...
class ComputeGasReactionGridKokkos : public ComputeGasReactionGrid, public KokkosBase {
 public:
  ...
  ComputeGasReactionGridKokkos(class SPARTA *, int, char **);
  ...
  // tally a single gas reaction in icell on device
  // reaction = 1 to N for which reaction, 0 = collision only (skipped)
  // this compute only tallies reacting collisions
                    ...
                    Particle::OnePart * /*kp*/) const
  {
    ...
    // tally the reaction to its grid cell
