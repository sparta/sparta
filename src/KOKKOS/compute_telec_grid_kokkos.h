...
ComputeStyle(telec/grid/kk,ComputeTelecGridKokkos)

#else

#ifndef SPARTA_COMPUTE_TELEC_GRID_KOKKOS_H
#define SPARTA_COMPUTE_TELEC_GRID_KOKKOS_H
...
struct TagComputeTelecGrid_compute_per_grid_atomic{};
struct TagComputeTelecGrid_compute_per_grid{};
struct TagComputeTelecGrid_post_process_grid{};

class ComputeTelecGridKokkos : public ComputeTelecGrid, public KokkosBase {
 public:
  ComputeTelecGridKokkos(class SPARTA *, int, char **);
  ~ComputeTelecGridKokkos();
  ...
  void operator()(TagComputeTelecGrid_compute_per_grid_atomic<NEED_ATOMICS>, const int&) const;

  KOKKOS_INLINE_FUNCTION
  void operator()(TagComputeTelecGrid_compute_per_grid, const int&) const;

  KOKKOS_INLINE_FUNCTION
  void operator()(TagComputeTelecGrid_post_process_grid, const int&) const;

  DAT::tdual_float_1d k_vector_grid;

 private:
  int nstride,count,eelec,nsp,index;
  double boltz;
