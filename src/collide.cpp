...
void Collide::init()
{
  ...
  if (ngroups != oldgroups) {
    ...
    memory->create(maxwigr, ngroups,"collide:maxwigr");                 // SWS
...
void Collide::collisions()
{
  ...
  if (!ambiflag) {
    ...
    if (ngroups == 1) {
      if (sws == 0) collisions_one_ambipolar();
      else collisions_one_ambipolar_SWS();   // SWS
      }
    else {
      if (sws == 0) collisions_group_ambipolar();
