...
void FixGridCheck::end_of_step()
{
  ...
  if (outflag == WARNING) {
    ...
    if (all && comm->me == 0) {
      char str[128];
      snprintf(str,sizeof(str),BIGINT_FORMAT " particles in wrong cells on timestep "
              BIGINT_FORMAT,all,update->ntimestep);
      error->warning(FLERR,str);
    }
    MPI_Allreduce(&nflag_surf,&all,1,MPI_SPARTA_BIGINT,MPI_SUM,world);
    if (all && comm->me == 0) {
      char str[128];
      snprintf(str,sizeof(str),BIGINT_FORMAT " particles inside surfs on timestep "
              BIGINT_FORMAT,all,update->ntimestep);
      error->warning(FLERR,str);
    }
    MPI_Allreduce(&nflag_split,&all,1,MPI_SPARTA_BIGINT,MPI_SUM,world);
    if (all && comm->me == 0) {
      char str[128];
      snprintf(str,sizeof(str),BIGINT_FORMAT " particles in wrong sub cells on timestep "
              BIGINT_FORMAT,all,update->ntimestep);
      error->warning(FLERR,str);
    }
  }
