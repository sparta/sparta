...
FixStyle(ablate/dev,FixAblateDev)

#else

#ifndef SPARTA_FIX_ABLATE_DEV_H
#define SPARTA_FIX_ABLATE_DEV_H

#include "fix.h"

namespace SPARTA_NS {

class FixAblateDev : public Fix {
 public:
  int igroup,dim;
  int nx,ny,nz;
  double cornerlo[3];
  ...
  FixAblateDev(class SPARTA *, int, char **);
  ~FixAblateDev();
  int setmask();
  void init();
  void setup() {}
 ...
 protected:
  ...
  class MarchingSquaresDev *ms;
  class MarchingCubesDev *mc;
