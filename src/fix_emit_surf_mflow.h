...
FixStyle(emit/surf_mflow,FixEmitSurfMflow)

#else

#ifndef SPARTA_FIX_EMIT_SURF_MFLOW_H
#define SPARTA_FIX_EMIT_SURF_MFLOW_H
...
class FixEmitSurfMflow : public FixEmit {
 public:
  FixEmitSurfMflow(class SPARTA *, int, char **);
  ~FixEmitSurfMflow();
  void init();
  void setup();
 ...
 private:
  ...
  int npertask,nthresh, valindex;
  double mflow,inlet_area,vmin,tsubsonic,nsubsonic;
  double tprefactor,soundspeed_mixture;
  double average_window,average_coef; // average_coef := 1/(average_window+1)
