...
#include "stdlib.h"
#include "string.h"
#include "fix_emit_surf_timeavg.h"
#include "update.h"
#include "domain.h"
#include "region.h"
#include "particle.h"
...
FixEmitSurfTimeAvg::FixEmitSurfTimeAvg(SPARTA *sparta, int narg, char **arg) :
...
FixEmitSurfTimeAvg::~FixEmitSurfTimeAvg()
{
...
void FixEmitSurfTimeAvg::grid_changed()
{
  create_tasks();
...
void FixEmitSurfTimeAvg::create_task(int icell)
{
...
void FixEmitSurfTimeAvg::perform_task()
{
...
void FixEmitSurfTimeAvg::subsonic_inflow()
{
...
void FixEmitSurfTimeAvg::subsonic_sort()
{
  int i,icell;
...
void FixEmitSurfTimeAvg::subsonic_grid()
{
  ...
  for (int i = 0; i < ntask; i++) {
    ...
    if (np) {
      ...
      // use cumulative average to stabilize vstream
...
void FixEmitSurfTimeAvg::grow_task()
{
  int oldmax = ntaskmax;
  ntaskmax += DELTATASK;
...
int FixEmitSurfTimeAvg::option(int narg, char **arg)
{
  if (strcmp(arg[0],"n") == 0) {
  ...
  if (strcmp(arg[0],"subsonic") == 0) {
    if (4 > narg) error->all(FLERR,"Illegal fix emit/face command");
    subsonic = 1;
    ...
    // check average window
    if (strcmp(arg[3],"NULL") == 0) average_window = 0.0;
    else {
      average_window = input->numeric(FLERR,arg[3]);
