...
# force rebuild of files with SPARTA_MPIIO switch
...
# edit 2 Makefile.package to include/exclude SPARTA_MPIIO setting

if (test $1 = 1) then

  if (test -e ../Makefile.package) then
    sed -i -e 's/[^ \t]*MPIIO[^ \t]* //' ../Makefile.package
    sed -i -e 's|^PKG_INC =[ \t]*|&-DSPARTA_MPIIO |' ../Makefile.package
