...
RestartMPIIO::RestartMPIIO(SPARTA *sparta) : Pointers(sparta)
{
  mpiio_exists = 1;
...
void RestartMPIIO::write(MPI_Offset headerOffset, int send_size, char *buf)
{
  bigint incPrefix = 0;
  bigint bigintSendSize = (bigint) send_size;
  MPI_Scan(&bigintSendSize,&incPrefix,1,MPI_SPARTA_BIGINT,MPI_SUM,world);

  bigint largestIncPrefix = incPrefix;
  MPI_Bcast(&largestIncPrefix, 1, MPI_SPARTA_BIGINT, (nprocs-1), world);

  int err = MPI_File_set_size(mpifh,
                              (headerOffset+(largestIncPrefix*sizeof(char))));
  if (err != MPI_SUCCESS) {
    char str[MPI_MAX_ERROR_STRING+128];
    char mpiErrorString[MPI_MAX_ERROR_STRING];
    int mpiErrorStringLength;
  ...
  err = MPI_File_write_at_all(mpifh,headerOffset +
                              ((incPrefix-bigintSendSize)*sizeof(char)),
                              buf,send_size,MPI_CHAR,MPI_STATUS_IGNORE);
  if (err != MPI_SUCCESS) {
    char str[MPI_MAX_ERROR_STRING+128];
    char mpiErrorString[MPI_MAX_ERROR_STRING];
    int mpiErrorStringLength;
...
void RestartMPIIO::read(MPI_Offset chunkOffset, bigint chunkSize, char *buf)
{
  int intChunkSize;
  ...
  while (remainingSize > 0) {
    ...
    int err = MPI_File_read_at(mpifh,currentOffset,&buf[bufOffset],
                               currentChunkSize,MPI_CHAR,MPI_STATUS_IGNORE);
    if (err != MPI_SUCCESS) {
      char str[MPI_MAX_ERROR_STRING+128];
      char mpiErrorString[MPI_MAX_ERROR_STRING];
      int mpiErrorStringLength;
