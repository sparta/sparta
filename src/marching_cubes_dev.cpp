...
int compare_indices_dev(const void *, const void *);
...
MarchingCubesDev::MarchingCubesDev(SPARTA *sparta, int ggroup_caller,
...
double MarchingCubesDev::interpolate(double v0, double v1, double lo, double hi)
{
...
void MarchingCubesDev::cleanup()
{
  ...
  qsort(dellist,ndelete,sizeof(int),compare_indices_dev);
...
int MarchingCubesDev::add_triangle(int *trig, int n)
{
  for(int t = 0; t < 3*n; t++) {
    switch (trig[t]) {
    case 0:
...
bool MarchingCubesDev::test_face(int face)
{
  double A,B,C,D;
...
bool MarchingCubesDev::test_interior(int s, int icase)
{
...
bool MarchingCubesDev::modified_test_interior(int s, int icase)
{
  int edge = -1;
  int amb_face;
...
int MarchingCubesDev::interior_ambiguity(int amb_face, int s)
{
  int edge;
...
int MarchingCubesDev::interior_ambiguity_verification(int edge)
{
...
bool MarchingCubesDev::interior_test_case13()
{
...
int compare_indices_dev(const void *iptr, const void *jptr)
{
  int i = *((int *) iptr);
  int j = *((int *) jptr);
  if (i < j) return 1;
...
void MarchingCubesDev::print_cube()
{
