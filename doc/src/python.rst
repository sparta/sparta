...
:doc:`variable <variable>` command for more details on variable styles
...
the :doc:`Python interface to SPARTA <Section_python>` doc page.  There
...
with proper indentation, blank lines, and comments, as desired.
...
itself using the :doc:`SPARTA Python module <Section_python>`.  This
...
as v\_name, where "name" is the name of the variable defined in the
...
argument, specified as iv\_name, where "name" is the name of the
...
defined, this command creates an :doc:`internal-style variable <variable>` with the specified name.
...
is "py\_varname(arg1,arg2,...argN)", where "varname" is the name of a
...
value.  The specified *varReturn* is of the form v\_name, where "name"
...
Note that it is permissible to use a :doc:`python-style variable <variable>` in a SPARTA command that allows for an
...
loaded into the Python "main" module, i.e.
...
line of the function imports the SPARTA Python module <Python\_module>.
...
The :doc:`Section python <Section_python>` doc page describes the syntax
...
(e.g. :doc:`fix custom <fix_custom>`), then the Python function will be
...
python          truncate return v\_foo input 1 iv\_arg format fi file truncate.py
variable        scalar equal py\_foo(vol)
...
as iv\_arg which is required to invoke the Python function from a
...
arguments c\_1[1] and c\_1[2] Python function wrapper are the cell
...
variable        xx particle py\_foo(x)
variable        yy particle py\_foo(y)
dump            2 particle all 1000 dump.particle.pyvar id x y v\_xx v\_yy
...
SPARTA.  Details on these steps are explained on the :doc:`Section python <Section_python>` doc page.  Note that it is important that
