...
The specified file can also (optionally) include a type for each
...
:doc:`surf\_collide <surf_collide>` command which affects how a particle
bounces off the surface.  Each surface element can optionally be
assigned to a reaction model, specified by the
:doc:`surf\_react <surf_react>` command which determines if any surface
...
surface elements (the default).  See the :doc:`global surfs <global>`
command to change this setting.

Note that the read\_surf command can be used multiple times to read
...
set of multiple files to exist.  The :doc:`write\_surf <write_surf>`
command explains how such sets are created.  Read\_surf will first read
...
information described below).  The read\_surf command then reads the
additional files.  For example, if the surface file was specified as
save.% when it was written, then read\_surf reads the files save.base,
...
:doc:`write\_surf <write_surf>` command for details.  The processors in
...
each line.  If not specified, the type of each line is set to 1.  Line
IDs and types can be used to assign lines to surface groups via the
:doc:`group surf <group>` command.
...
The *custom* values are optional and can only be included in the file
if the *custom* keyword is used one or more times.  Each use of the
*custom* keyword determines how many additional values are appended to
each line in the *Lines* section.  For a custom per-surf vector, a
single value is appended.  For a custom per-surf array, Nc values are
appended.  The values are assigned to custom vectors or arrays in the
order the *custom* keywords are specified.  For example, for this
read\_surf command, 4 custom values must be added to the end of each
...
per-surf vector named temperature which stores a single value per
...
groups via the :doc:`group surf <group>` command.
...
keyword determines how many additional values are appended to each
line in the *Triangles* section.  For a custom per-surf vector, a
single value is appended.  For a custom per-surf array, Nc values are
appended.  The values are assigned to custom vectors or arrays in the
order the *custom* keywords are specified.  For example, for this
read\_surf command, 4 custom values should be added to the end of each
...
The first floating-point value will be the assigned to a custom
per-surf vector named temperature which stores a single value per
triangle.  The next 3 integers will be assigned to a custom per-surf
array named flags which stores 3 values per triangle.
...
output by the :doc:`dump surf <dump>` command and uses as inputs by
other commands.  For example, many of the models for the
:doc:`surf\_collide <surf_collide>` command take temperature as an input;
...
If the read\_surf command is used multiple times and the same custom
...
See the :doc:`group surf <group>` command for details.
...
:doc:`remove\_surf <remove_surf>` was used to remove a surface object, and
...
optional "\*" and "%" wildcard characters as well as the write\_surf
...
created by the :doc:`create\_grid <create_grid>` command.  If particles
