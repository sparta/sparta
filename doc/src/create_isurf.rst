...
:doc:`read\_isurf <read_isurf>` command which reads a file of corner point
...
points.  See the :doc:`read\_isurf <read_isurf>` command for details.
...
:doc:`read\_isurf <read_isurf>` doc page has additional information about
...
As with the "read\_isurf" command, the algorithm to create the implicit
...
explicit surfaces must already be defined by the :doc:`read\_surf <read_surf>`
command.  Simulations with implicit surfaces cannot perform grid
adaptation.

The :doc:`global surfs explicit/distributed <global>` command must be
used before using the :doc:`read\_surf <read_surf>` command which defined
