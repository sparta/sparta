...
each species are set in the file read by the :doc:`species <species>`
command.
...
:doc:`react <react>` command, particles involved in collisions may also
change species, or a particle may be deleted, or a new particle
created.  The :doc:`collide\_modify <collide_modify>` command can also be
...
:doc:`species <species>` command.  The group definitions in the mixture
...
defined by the simulation, via the :doc:`species <species>` command.
...
species data file, as read by the :doc:`species <species>` command.
