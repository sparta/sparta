...
that surface/boundary by the :doc:`surf\_modify collide <surf_modify>`
command.

The sum of *pdelete* and *pcreate* must be <= 1.0.

Note that if you simply wish to delete all particles which hit the
surface, you can use the :doc:`surf\_collide vanish <surf_collide>`
...
simulation is run via the :doc:`run <run>` command, the list is scanned.
...
surface/boundary, listed as a :doc:`species <species>` IDs.  P1 and P2
are one or two products, also listed as :doc:`species <species>` IDs.
...
:doc:`surf\_react\_adsorb <surf_react_adsorb>` doc page.
...
:doc:`surf\_react\_adsorb <surf_react_adsorb>`
