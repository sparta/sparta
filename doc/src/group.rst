...
with the *region-ID* into the group.  See the :doc:`region <region>`
command for details on what kind of geometric regions can be defined.
Note that the *side* option for the :doc:`region <region>` command can
...
:doc:`read\_surf <read_surf>` command.  The ordering of IDs is determined
...
:doc:`region <region>` command for details on what kind of geometric
regions can be defined.  Note that the *side* option for the
:doc:`region <region>` command can be used to define whether the inside
