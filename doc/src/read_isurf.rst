...
* ablateID = ID of a :doc:`fix ablate <fix_ablate>` command
...
a different kind of file.  See the `Howto 6.13 <Section_howto.html#howto_13>`_ section of the manual for an
...
Note that at some point, it will be possible to use the read\_isurf
...
The tools directory contains a implicit\_grid.py tool which can create
...
:doc:`write\_isurf <write_isurf>` command.  The latter is typically used
when running an ablation model via the :doc:`fix ablate <fix_ablate>`
...
symmetry axis.  A body of revolution legitimately closes on itself
there, so if the array of grid cells starts on the axis, corner point
values on that y = 0 boundary are allowed to be non-zero.  The
resulting surface end points lie on the box boundary, which the
watertight check permits.  If the array of grid cells starts above the
axis, the usual requirement that the outer boundary corner point values
be 0 still applies.
...
read\_isurf is not an exact inverse of a :doc:`write\_isurf <write_isurf>`.
When you use the write\_isurf then read\_isurf sequence to save and
restore a partially ablated (mid-run) surface state, as described on
the :doc:`write\_isurf <write_isurf>` doc page, use *push no* so the
