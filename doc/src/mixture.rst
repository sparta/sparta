...
:doc:`create\_particles <create_particles>` or :doc:`per-grid computes <compute>`.  Any number of mixtures can be defined and
used in a simulation.

A mixture is a collection of one or more particle species as defined
by the :doc:`species <species>` command.  Each species belongs to a
...
As described by the :doc:`collide <collide>` command, mixture groups are
...
efficiency.  :doc:`Per-grid computes <compute>` also use mixture groups
...
:doc:`global <global>`, :doc:`create\_particles <create_particles>`

**Default:**

The *nrho*\ , *vstream*\ , and *temp* defaults are those defined for the
background gas density, as set by the :doc:`global <global>` command.
