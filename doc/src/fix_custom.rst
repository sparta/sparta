...
:doc:`custom <custom>` command with two differences.
...
cannot be used with the :doc:`custom <custom>` command.

All of the actions supported by this command reset the values of
custom attribute(s) which must already exist.  See the
:doc:`custom <custom>` and :doc:`read\_surf <read_surf>` and
:doc:`read\_grid <read_grid>` commands for different ways to define and
...
output by a :doc:`dump <dump>` command on the same timestep, that takes
...
:doc:`run <run>` commands.  For example, if *Nfreq* = 1000 and the input
