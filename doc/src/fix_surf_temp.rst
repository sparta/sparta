...
e.g. *etot* from the :doc:`compute surf <compute_surf>` command.  Note
...
:doc:`surf\_collide diffuse <surf_collide>` command, as shown above.

The per-surface element temperatures computed by this fix can be
output via the :doc:`dump surf <dump>` command, using its *s\_name*
syntax.  See the examples/adjust\_temp directory for input scripts that
use this fix.
...
compute, such as the :doc:`compute surf <compute_surf>` command.  Or it
can be specified a per-surf quantity calculated by a fix, e.g. one
which performs time-averaging of per-surf quantities, such as :doc:`fix ave/surf <fix_ave_surf>`.

If the specified *source* has no bracketed term appended, the compute
or fix must calculate a per-surf vector.  If *c\_ID[N]* or
*f\_ID[N]* is used, then N must be in the range from 1-M, which will
...
where q\_wall is the heat flux to the surface (provided by the compute
or fix), sigma is the Stefan-Boltzmann constant appropriate to the
:doc:`units <units>` being used, *emisurf* is the surface emissivity,
...
E.g. the :doc:`surf\_collide diffuse <surf_collide>` command is assigned
to a group of surface elements via the :doc:`surf\_modify <surf_modify>`
...
:doc:`surf\_collide diffuse <surf_collide>` and multiple fix surf/temp
...
command.  As explained on the :doc:`read\_restart <read_restart>` doc
