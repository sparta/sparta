...
simulation by using the :doc:`fix adapt <fix_adapt>` command.
...
becomes a single new child cell.  See `Section howto 6.8 <Section_howto.html#howto_8>`_ for a description of the hierarchical
...
all its child cells are in the specified grid group.  See the :doc:`group grid <group>` command for info on how grid cells can be assigned to
...
adaptation if the adapt\_grid command is invoked again or successive
adaptations are performed via the :doc:`fix adapt <fix_adapt>` command.
...
should use the *value* style with the ID of a :doc:`fix ave/grid <fix_ave_grid>` command that time-averages particle counts
from the :doc:`compute grid <compute_grid>` command.
...
The *value* style uses values calculated by a :doc:`compute <compute>`
or :doc:`fix <fix>` to decide whether to adapt each cell.  The fix or
compute must calculate per-grid values as described in `Section howto 6.4 <Section_howto.html#howto_4>`_.  If the compute or fix calculates a
vector of such values, it is specified as c\_ID or f\_ID.  If it
calculates an array of such values, it is specified as c\_ID[N] or
f\_ID[N] when N is the column of values to use, from 1 to Ncolumns.
...
Here is an example using particle count as calculated by the :doc:`compute grid <compute_grid>` command as an adaptation criterion.  A cell
...
:doc:`compute lambda/grid <compute_lambda_grid>` command as an
...
`Section howto 6.8 <Section_howto.html#howto_8>`_ for a definition of the
...
See `Section howto 6.8 <Section_howto.html#howto_8>`_ for a definition of
...
replaced by the current timestep.  This is useful for the :doc:`fix adapt <fix_adapt>` command, if you wish to write out multiple grid
...
remain un-clumped after adaptation.  See `Section howto 6.8 <Section_howto.html#howto_8>`_ for a description clumped and
unclumped grids.
