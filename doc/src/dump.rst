...
* style = *particle* or *grid* or *surf* or *tally* or *image*
* select-ID = which particles, grid cells, surface elements to dump
  ...
  .. parsed-literal::
       ...
       for style = image, see the :doc:`dump image <dump_image>` doc page
...
* args = list of arguments for a particular style
  ...
  .. parsed-literal::
           ...
           c_ID = per-tally vector calculated by a compute with ID
...
configuration every N timesteps, as discussed on the :doc:`dump image <dump_image>` doc page.
...
and in what format.  The *particle*\ , *grid*\ , *surf*\ , *tally* options
are for particles, grid cells, surface elements, or tallies.  Settings
made via the :doc:`dump\_modify <dump_modify>` command can also alter
...
*select-ID* is a mixture ID as defined by the :doc:`mixture <mixture>`
...
file.  Many of the SPARTA :doc:`post-processing tools <Section_tools>`,
including `Pizza.py <https://lammps.github.io/pizza>`_, work with this
format.
...
See the :doc:`boundary <boundary>` command for details.
...
tmp.dump.20000, etc.  Note that the :doc:`dump\_modify pad <dump_modify>`
...
to convert it back to text format (see the binary2txt tool) or write your own code to read the
...
*Id* is the particle ID.  *Type* is an integer index for the particle
...
command.  *Proc* is the ID of the processor which currently owns the
particle.

The *x*\ , *y*\ , *z* attributes write particle coordinates "unscaled", in
the appropriate distance :doc:`units <units>`.  Use *xs*\ , *ys*\ , *zs* to
...
affected by the :doc:`collide\_modify <collide_modify>` command.

The *c\_ID* and *c\_ID[N]* attributes allow per-particle vectors or
arrays calculated by a :doc:`compute <compute>` to be output.  The ID in
...
:doc:`compute <compute>` command for details.

If *c\_ID* is used as a attribute, the compute must calculate a
per-particle vector, and it is output.  If *c\_ID[N]* is used, the
...
The *f\_ID* and *f\_ID[N]* attributes allow vector or array
per-particle quantities calculated by a :doc:`fix <fix>` to be output.
...
If *f\_ID* is used as a attribute, the fix must calculate a
per-particle vector, and it is output.  If *f\_ID[N]* is used, the
...
The *v\_name* attribute allows per-particle vectors calculated by a
...
If *p\_name* is used as a attribute, the custom attribute must be a
vector, and it is output.  If *p\_name[N]* is used, the custom
...
See :doc:`Section 10 <Section_modify>` of the manual for information on
...
:doc:`units <units>`.  The *xhi*\ , *yhi*\ , *zhi* attributes write the
...
The *c\_ID* and *c\_ID[N]* attributes allow per-grid vectors or arrays
calculated by a :doc:`compute <compute>` to be output.  The ID in the
attribute should be replaced by the actual ID of the compute that has
been defined previously in the input script.  See the
:doc:`compute <compute>` command for details.

If *c\_ID* is used as a attribute, and the compute calculates a
per-grid vector, then the per-grid vector is output.  If *c\_ID[N]*
...
The *f\_ID* and *f\_ID[N]* attributes allow per-grid vectors or arrays
...
The *v\_name* attribute allows per-grid vectors calculated by a
:doc:`variable <variable>` to be output.  The name in the attribute
should be replaced by the actual name of the variable that has been
...
If *g\_name* is used as a attribute, the custom attribute must be a
vector, and it is output.  If *g\_name[N]* is used, the custom
attribute must be an array, and N must be in the range from 1-M, which
will output the Nth column of an M-column array.  See the discussion
above for how N can be specified with a wildcard asterisk to
effectively specify multiple values.
...
The *c\_ID* and *c\_ID[N]* attributes allow per-surf vectors or arrays
calculated by a :doc:`compute <compute>` to be output.  The ID in the
attribute should be replaced by the actual ID of the compute that has
been defined previously in the input script.  See the
:doc:`compute <compute>` command for details.
...
If *c\_ID* is used as a attribute, and the compute calculates a
per-surf vector, then the per-surf vector is output.  If *c\_ID[N]*
is used, then N must be in the range from 1-M, which will output the
Nth column of the M-column per-surf array calculated by the compute.
See the discussion above for how N can be specified with a wildcard
asterisk to effectively specify multiple values.

The *f\_ID* and *f\_ID[N]* attributes allow per-surf vectors or arrays
calculated by a :doc:`fix <fix>` to be output.  The ID in the attribute
should be replaced by the actual ID of the fix that has been defined
previously in the input script.

If *f\_ID* is used as a attribute, and the fix calculates a per-surf
vector, then the per-surf vector is output.  If *f\_ID[N]* is used,
then N must be in the range from 1-M, which will output the Nth column
of the M-column per-surf array calculated by the fix.  See the
discussion above for how N can be specified with a wildcard asterisk
to effectively specify multiple values.

The *v\_name* attribute allows per-surf vectors calculated by a
:doc:`variable <variable>` to be output.  The name in the attribute
should be replaced by the actual name of the variable that has been
...
If *s\_name* is used as a attribute, the custom attribute must be a
vector, and it is output.  If *s\_name[N]* is used, the custom
attribute must be an array, and N must be in the range from 1-M, which
will output the Nth column of an M-column array.  See the discussion
above for how N can be specified with a wildcard asterisk to
effectively specify multiple values.
...
This section explains the tally attributes that can be specified as
part of the *tally* style.

The *c\_ID* and *c\_ID[I]* attributes allow per-tally vectors or
arrays calculated by a :doc:`compute <compute>` to be output.  The ID in
the attribute should be replaced by the actual ID of the compute that
has been defined previously in the input script.  See the
:doc:`compute <compute>` command for details.
...
If *c\_ID* is used as a attribute, and the compute calculates a
per-tally vector, then the per-tally vector is output.  If *c\_ID[I]*
