...
:doc:`mixture <mixture>` command.  Any or all them can be overridden by
...
:doc:`mixture <mixture>` command.  These can also be overridden by
...
:doc:`collide\_modify <collide_modify>` command.  Note that if the
:doc:`collide <collide>` command has not been specified (free molecular
...
defined by the :doc:`mixture <mixture>` command.  These are the flow
...
itself, as set by the :doc:`mixture <mixture>` command.  However, for
...
the sum = 1.0, as explained on the :doc:`mixture <mixture>` command doc
...
keyword is used for the :doc:`fix emit/face <fix_emit_face>` command.
If just the *press* keyword is specified, but not the *temp* keyword,
then it is similar to the "subsonic press NULL" setting for the :doc:`fix emit/face <fix_emit_face>` command.  If both keywords are used it
is similar to the "subsonic press temp" setting for the :doc:`fix emit/face <fix_emit_face>` command.  The difference with this
...
and Liou `(Fang02) <#Fang02>`_ to determine the number of particles to
...
equation 4.22 of `(Bird94) <#Bird94>`_.
...
relative number fraction of the species, as set by the :doc:`mixture nfrac <mixture>` command.  If *M* has a fractional value,
...
that the *side* option for the :doc:`region <region>` command can be
