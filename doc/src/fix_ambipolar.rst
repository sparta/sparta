...
:doc:`species <species>` command), and the ions have positive charges.
...
particle.  As explained on the :doc:`read\_restart <read_restart>` doc
...
script before the first :doc:`run <run>` command is used, then the
...
can be accessed by the :doc:`dump particle <dump>` command, as p\_ionambi
and p\_velambi.  That means those per-particle values can be written to
particle dump files.
