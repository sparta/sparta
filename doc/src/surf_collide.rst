...
Second, *Tsurf* can be a variable specified as v\_name where *name* is
...
outputs per-surf quantities.  For example the :doc:`compute surf <compute_surf>` and :doc:`fix ave/surf <fix_ave_surf>` commands
...
Third, *Tsurf* can be a custom per-surf attribute specified as s\_name
...
time they are modified (e.g. by the :doc:`fix surf/temp <fix_surf_temp>`
command), then their use by a surf\_collide model will trigger
...
with specular reflection, via the :doc:`surf\_react <surf_react>`
command, which cannot be done in conjunction with the surf\_collide
specular command.  See the :doc:`surf\_react <surf_react>` doc page for
details.
...
*Tsurf*\ , *acc\_n*, *acc\_t*, *acc\_rot*, and *acc\_vib* arguments.

The *Tsurf* argument specifies the temperature of the surface.  It can
be a numeric value, surf-style variable, or custom per-surf attribute.
See the explanation for all 3 options above.

*Acc\_n*, *acc\_t*, *acc\_rot*, and *acc\_vib* are the accommodation
...
*eff\_mass* specifying the effective mass of the surface atom. The
...
other five parameters *Tsurf*\ , *var*\ , *pol\_peak*, *pol\_pow*, *azi\_pow*
...
*pol\_peak* is the peak of the polar angle distribution; *pol\_pow* and
*azi\_pow* are the cosine power representing the polar and azimuthal
...
:doc:`bound\_modify <bound_modify>` command or to surface elements which
...
simulation domain, e.g. using the :doc:`fix emit/surf <fix_emit_surf>`
command.  Using this surface collision model will also treat the
surface as an outflow boundary.  This is similar to using the :doc:`fix emit/face <fix_emit_face>` command on a simulation box face while
also setting the face to be an outflow boundary via the :doc:`boundary o <boundary>` command.

Note that the :doc:`surf\_react global <surf_react>` command can also be
used to delete particles hitting a surface, by setting the *pdelete*
parameter to 1.0.  Using a surf\_collide vanish command is simpler.
...
unit, where time units are defined by the :doc:`units <units>` command.
...
*v\_name*.  The *Nfreq* value determines the frequency at which the
...
requires one argument *pol\_pow\_2*, which describes the distribution
between the peak and the surface. The distribution between the surface
normal and the peak is described using the parameter *pol\_pow*.
