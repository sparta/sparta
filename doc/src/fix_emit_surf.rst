...
:doc:`group surf <group>` is used to define surface groups.
...
options of the :doc:`collide\_modify <collide_modify>` command.  Note
that if the :doc:`collide <collide>` command has not been specified
...
set by the :doc:`mixture nfrac <mixture>` command.  If *M* has a
...
for the :doc:`region <region>` command can be used to define whether the
...
For example, the :doc:`read\_surf <read_surf>` command can read per-surf
attributes included in a surface data file.  The :doc:`custom <custom>`
and :doc:`fix custom <fix_custom>` commands allow for definition of
custom per-surf vectors or arrays and their initialization by use of
:doc:`surf-style variables <variable>` or by reading the values from a
file.  Note that the use of "fix custom" allows for the resetting of
custom attributes once every N timesteps, which allows for
...
* nrho = number density (# per volume or area units) = per-surf vector
...
The *s\_name* value of the *custom* keyword is the name of the custom
...
values of the associated custom per-surf vector or array override the
default mixture properties.

The *custom* attribute *nrho* sets a number density used to calculate
the count of particles emitted from each surface element.  The nrho
values should be in per-volume units = per-length\^3 for 3d models
or axisymmetric models.  The values should be in per-area units =
...
per-surf array, is the same as the order of species within the mix-ID
...
If *normal* is set to *no*\ , which is the default, then unlike the :doc:`fix emit/face <fix_emit_face>` command, no warning is issued if a
