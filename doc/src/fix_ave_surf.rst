...
* value = c\_ID, c\_ID[N], f\_ID, f\_ID[N], v\_name, s\_name, s\_name[N]
...
:doc:`dump surf <dump>` command.  Only surface elements in the surface
...
:doc:`fix <fix>` or :doc:`surf-style variable <variable>` or a custom
...
then see the :doc:`fix ave/time <fix_ave_time>` command.  To
time-average per-grid quantities, see the :doc:`fix ave/grid <fix_ave_surf>` command.

Each per-surf value of each input vector is averaged independently.

:doc:`Computes <compute>` that produce per-surf vectors or arrays are
...
equivalent, since the :doc:`compute surf <compute_grid>` command creates
...
*c\_ID[N]* is used, the compute must calculate a per-surf array with
...
previously defined in the input script.  If no bracketed term is
appended, the fix must calculates a per-surf vector.  If *f\_ID[N]*
...
If *s\_name* is used as a value, the custom attribute must be a vector.
If *s\_name[N]* is used, the custom attribute must be an array, and N
...
formulas discussed on the :doc:`compute surf <compute_surf>` doc page,
