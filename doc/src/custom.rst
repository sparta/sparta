...
* action = *create* or *remove* or *set* or *file*
...
The specified *v\_name* is the name of the variable which is evaluated.
...
:doc:`compute <compute>` or :doc:`fix <fix>` commands.  They can also
...
value.  See the :doc:`mixture <mixture>` and :doc:`group <group>` commands
...
:doc:`region <region>` command.  Only particles or grid cells or surface
...
* If two or more coarse grid points are the "same" nearest distance from
  ...
  point. I.e. one part in a million.
* SPARTA center points will never be on the boundary of the simulation
  ...
  be inside the SPARTA simulation box or on its surface.
...
or arrays by this command.  Using this action with the :doc:`fix custom <fix_custom>` command could assign per-grid custom values
...
surf.3.attribute, etc will be read.  Note that if the :doc:`fix custom <fix_custom>` command uses this action, and there are also
...
.. parsed-literal::
   ...
   Nvaluecustom values (Nvalue 8-byte doubles for Ncoarse cell)
